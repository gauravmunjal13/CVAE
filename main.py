...
if __name__ == '__main__':
    ...
    # possible options are "cvae" and "ncf"
    parser.add_argument("--model_type", dest="model_type", type=str, default="cvae")
    ...
    data_config["file_name"] = "training_slate_plays_slate_10.csv"
    ...
    data_config["file_name"] = "val_slate_plays_slate_10.csv"
    ...
    exp_logs = os.path.join(model_config["model_dir"], "output_logs/analysis12/")
    if not os.path.exists(exp_logs):
        os.mkdir(exp_logs)
        
    # create folder to save the model
    exp_saved_models = os.path.join(model_config["model_dir"], "saved_models/analysis12/")
    ...
    if args.exp_type == "evaluate":
        eval_config = {
            ...
            "exp_save_models_dir": "./experiments/cvae/saved_models/analysis12/",
            "slate_size": 5 # for different slate size
        }
        # Is there a better way to wrap large num of arguments
        precision, recall, user_test_metric = evaluate.evaluation(model, args, eval_config)
